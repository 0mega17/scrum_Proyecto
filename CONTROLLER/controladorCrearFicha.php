...
if (
    ...
    $consultaId = "select count(id) from fichas where id = '$codigoFicha'";

    $resultadoId = $Mysql->efectuarConsulta($consultaId);
    if ($resultadoId && mysqli_num_rows($resultadoId) > 0) {
        echo json_encode(
            [
                "susses" => false,
                "mensaje" => "El id ingresado ya existe ingrese otro",
            ]
        );
        exit;
    }
